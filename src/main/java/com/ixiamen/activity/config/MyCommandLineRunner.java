...
public class MyCommandLineRunner implements CommandLineRunner {
    ...
    private void doScanner(String packageName) {
        //把所有的.替换成/
        URL url = this.getClass().getClassLoader().getResource(packageName.replaceAll(Constant.ESCAPE_POINT, "/"));
        // 是否循环迭代
        assert url != null;
        if (StringUtils.countMatches(url.getFile(), ".jar") > 0) {
            ...
            try {
                ...
                while (entries.hasMoreElements()) {
                    ...
                    // 如果前半部分和定义的包名相同
                    if (name.startsWith(packageName.replaceAll(Constant.ESCAPE_POINT, "/"))) {
