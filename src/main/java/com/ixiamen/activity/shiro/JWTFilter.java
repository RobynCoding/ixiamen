...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    protected boolean preHandle(ServletRequest request, ServletResponse response) throws Exception {
        ...
        String authorization = httpServletRequest.getHeader("Authorization");
        logger.debug("请求地址：" + httpServletRequest.getRequestURI());
    ...
    /**
     * 验证请求方法是否有@Pass注解,有则直接放行，不管authorization是否为空
     */
    private boolean verificationPassAnnotation(ServletRequest request, ServletResponse response, HttpServletRequest httpServletRequest, String authorization) {
    ...
    private void responseError(ServletRequest request, ServletResponse response) {
        ...
        try {
            ...
            out.print(JSONObject.toJSONString(ResponseHelper.unauthorizedFailure(PublicResultConstant.UNAUTHORIZED)));
            out.flush();
        } catch (Exception e) {
            logger.error("非法url返回身份错误信息报错", e);
        } finally {
            if (out != null) {
                out.close();
            }
